...
void main() {
  ...
  testWidgets('only the applications with the selected status are listed',
      (tester) async {
    seedApplications();

    await tester.pumpWidget(createList(status: 'applied'));
  ...
  testWidgets('switching the status shows the other applications',
      (tester) async {
    seedApplications();

    await tester.pumpWidget(createList(status: 'accepted'));
    await tester.pumpAndSettle();
  ...
  testWidgets('an empty status shows an explicit message', (tester) async {
    seedApplications();

    await tester.pumpWidget(createList(status: 'refused'));
    await tester.pumpAndSettle();

    expect(find.text('No refused applications'), findsOneWidget);
  });

  testWidgets('the salary of the offer is displayed', (tester) async {
    seedApplications();
  ...
  testWidgets('a logged out user is told to sign in', (tester) async {
    await tester.pumpWidget(
      createList(status: 'applied', auth: FakeAuthProvider(authService)),
    );
    await tester.pumpAndSettle();
